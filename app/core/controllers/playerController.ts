...
uploads3 : uploads3
  ...
  y.performance = performance
      ...
      delete stats.player
      const sql = 'insert into stats(tou_key ,player_name, jersey_name, player_key , legal_name,gender , date_of_birth, nationality , team_key , seasonal_role , performance) values ?';
      let d = []
      let p = []
...
p.push([el.key, el.name , el.jersey_name ,  el.legal_name ,el.gender ,el.date_of_birth  , JSON.stringify(el.nationality)])
     ...
     await this.addPlayer(p)
      await this.connection.write.query(sql , [d])
